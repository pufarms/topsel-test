...
export default function OrdersPrintWaybillPage() {
  ...
  const filteredOrders = useAdminCategoryFilter(printableOrders, filters, getFields);
              ...
              <Table className="min-w-[1800px]">
                <TableHeader className="sticky top-0 z-10 bg-background">
                  <TableRow>
                    ...
                    <TableHead className="min-w-[300px]">배송지</TableHead>
                    <TableHead className="w-[150px]">배송메시지</TableHead>
                ...
                <TableBody>
                  {filteredOrders.map((order, index) => (
                    <TableRow key={order.id}>
                      ...
                      <TableCell className="max-w-[300px] truncate" title={order.recipientAddress || ""}>
                        {order.recipientAddress || "-"}
                      </TableCell>
                      <TableCell className="max-w-[150px] truncate" title={order.deliveryMessage || ""}>
                        {order.deliveryMessage || "-"}
                      </TableCell>
                      <TableCell className="font-mono">{order.trackingNumber || "-"}</TableCell>
                      <TableCell>
                        <Badge 
                          className={order.status === "배송중" ? "bg-orange-500" : "bg-blue-500"}
                        >
                          {order.status}
                        </Badge>
                      </TableCell>
