...
function VideoGallerySection({ data, sectionId, isEditing, onClick, onFieldEdit }: SectionProps) {
  ...
  return (
    <section className="ts-section-dark" style={{ paddingTop: '50px', paddingBottom: '80px', background: 'linear-gradient(135deg, #111827 0%, #1f2937 100%)' }} data-testid="section-video-gallery">
      {/* Video Modal */}
      {activeVideoId && (
        <div 
        ...
        >
          {data.subtitle && (
            <EditableField
              ...
              style={{ color: 'var(--ts-accent-cyan)', marginBottom: '10px' }}
            />
          )}
          {data.title && (
            <EditableField
...
function IconCardsSection({ data, sectionId, isEditing, onClick, onFieldEdit }: SectionProps) {
  ...
  const sectionStyle: React.CSSProperties = {
    ...
    background: isLight ? undefined : 'linear-gradient(135deg, #111827 0%, #1f2937 100%)',
  };
  
  return (
    <section className={isLight ? 'ts-section-light' : ''} style={sectionStyle} data-testid="section-icon-cards">
      <div className="container">
        <div 
          ref={anim.ref}
          className={`transition-all duration-700 ${anim.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
        >
          <div className="text-center mb-12 flex flex-col items-center" style={{ gap: 0 }}>
            {data.subtitle && (
              <span 
                className="subtitle-label"
                style={{ 
                  color: isLight ? 'var(--ts-primary)' : 'var(--ts-accent-cyan)', 
                  marginBottom: '10px',
                  display: 'block'
                }}
  ...
  return (
    <section
      style={{ 
        ...
        paddingTop: data.paddingTop || '80px',
        paddingBottom: '60px'
      }}
        ...
        >
          ...
          {data.promoBadge && (
            <a 
              href={data.promoBadgeLink || "#"}
              className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium"
              style={{ background: 'rgba(255,255,255,0.15)', color: 'white', border: '1px solid rgba(255,255,255,0.3)', marginBottom: '50px' }}
            >
