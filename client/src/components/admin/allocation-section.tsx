...
export default function AllocationSection() {
  ...
  useEffect(() => {
    if (!isLoading && allocationsData && allocationsData.allocations.length === 0 && selectedDate && autoGeneratedRef.current !== selectedDate) {
      autoGeneratedRef.current = selectedDate;
      generateMutation.mutate();
    }
  }, [isLoading, allocationsData, selectedDate]);
          ...
          <div className="space-y-3">
            ...
            {additionalVendorInputs.length === 0 ? (
              ...
              <>
                ...
                {additionalVendorInputs.filter(vi => vi.vendorId !== 0).length > 0 && (
                  <div className="text-xs text-muted-foreground pt-1 border-t">외주 업체</div>
                )}
                {additionalVendorInputs.filter(vi => vi.vendorId !== 0).map((vi) => {
                  const realIdx = additionalVendorInputs.findIndex(v => v.vendorId === vi.vendorId);
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-3">
            ...
            {(() => {
              ...
              const totalAllocated = confirmInputs.reduce((sum, c) => sum + c.allocatedQuantity, 0) + (hasSelfInDetails ? 0 : selfQuantity);
