    ...
    try {
      ...
      for (const row of paymentRows) {
        items.push({
          ...
          companyName,
          productName: `[입금] ${(row.memo && row.memo.trim()) ? row.memo.trim() : ({ transfer: '계좌이체', card: '카드', product_offset: '상품상계' } as Record<string, string>)[row.paymentMethod || ''] || row.paymentMethod || ''}`,
          productCode: "",
          quantity: 0,
          unitPrice: 0,
          subtotal: 0,
  ...
  app.post('/api/admin/accounting/popbill-issue', async (req, res) => {
    try {
      ...
      const taxinvoice = {
        ...
        invoicerTaxRegID: '',
        invoicerCorpName: '현 농업회사법인 주식회사',
        invoicerCEOName: '공강렬',
        invoicerAddr: '경상북도 의성군 금성면 청로길 175',
        invoicerBizType: '도매및 소매업',
        invoicerBizClass: '농산물유통,가공, 판매업,통신판매업',
        invoicerContactName: '공강렬',
        invoicerEmail: 'kgong5026@gmail.com',
        invoicerTEL: '1588-8707',
        invoiceeType: '사업자',
