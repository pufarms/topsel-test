...
비용 등록: item_name="농협 가마니" → category="물류/배송비" (관리자가 수동 선택)
    ↓
학습 처리:
  1. expense_keywords에 "농협 가마니" 키워드가 있는지 확인
  2. 없으면 → INSERT (keyword: "농협 가마니", category: "물류/배송비", priority: 50, source: "learned")
  3. 있으면 → use_count +1, last_amount 업데이트
  4. "농협"이라는 단어도 별도로 INSERT (priority: 15, source: "learned")
    ↓
다음번: "농" 입력 → "농협 가마니 🚚물류/배송비" 자동 표시
```
...
6. ✅ 키워드 학습 로직: 수동 분류 선택 시 expense_keywords에 자동 추가되는가?
7. ✅ 과세/면세에 따라 공급가/부가세가 자동 계산되는가?
8. ✅ 정기 비용 생성 시 중복 방지가 동작하는가?
...
22. ✅ 월간 요약 카드에 분류별 금액이 표시되는가?
23. ✅ 도넛차트 + 라인차트가 정상 렌더링되는가?
24. ✅ 엑셀 업로드/다운로드가 동작하는가?
